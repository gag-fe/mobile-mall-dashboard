...
const SaleTrend = React.createClass({
  getInitialState(){
    return {
      selectedIndex: 0,
  ...
  componentWillReceiveProps(nextProps){
      if (this.props.saleTrend != nextProps.saleTrend) {
        this.setState({
          saleTrend: nextProps.saleTrend
        });
    }
  },

  render(){
    if(this.state.saleTrend.length == 0){
      return false;
    }
    var newSaleTrend = this._getSaleTrendData(this.state.saleTrend);

    return (
        <div>
      <div><h2>销售额走势</h2></div>
        <Chart options={newSaleTrend} onReady={this._chartsReadyLine}></Chart>
      </div>
    );
  }
});
